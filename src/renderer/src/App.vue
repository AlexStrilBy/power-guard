...
<template>
  <div v-if="view==='settings'" class="wrap">
    ...
    <div class="row">
      ...
      <input type="number" v-model.number="s.failureSeconds" min="2" max="120" />
    </div>
    ...
    <div class="row">
      ...
      <input type="number" v-model.number="s.confirmCountdown" min="5" max="300" />
    </div>

    <div class="row">
      <label>Snooze (minutes) after cancel</label>
      <input type="number" v-model.number="s.snoozeMinutes" min="1" max="120" />
    </div>

    <div class="row checkbox">
      <label><input type="checkbox" v-model="s.enabled" /> Enable monitoring</label>
    </div>
    <div class="row checkbox">
      <label><input type="checkbox" v-model="s.startWithWindows" /> Start with Windows</label>
    </div>
  ...
  <div v-else class="confirm">
    ...
    <p>Action: <b>{{ confirm.action }}</b> in <b>{{ confirm.remain }}</b> seconds…</p>
    <div class="actions">
...
<style scoped>
.wrap { padding: 16px; font-family: ui-sans-serif, system-ui; }
.row { margin-top: 12px; display: grid; gap: 6px; }
.row.checkbox { display: block; }
.row.actions { margin-top: 16px; }
input, select { padding: 6px 8px; }
button { padding: 8px 12px; cursor: pointer; }
button.secondary { margin-left: 8px; }
.hint { margin-top: 12px; color: #666; font-size: 12px; }
.confirm { padding: 20px; text-align: center; font-family: ui-sans-serif, system-ui; }
</style>
